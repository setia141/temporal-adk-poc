...
CLARIFY_CONVENTION = (
    ...
    "whenever you're not confident, not a last resort. Concretely: if a "
    "field your task actually depends on is a placeholder like 'TBD', "
    "'N/A', 'unknown', 'not sure', empty, or is genuinely too vague to "
    "act on, that counts as missing — you must ask about it rather than "
    "writing it through as-is or inventing a plausible-sounding value for "
    "it.\n\n"
    "But you will often be given more context than your specific task "
    "actually needs — earlier-stage output, background info, fields "
    "another agent already handled. A placeholder or unknown value "
    "there is not your problem to raise, even if it's technically "
    "'incomplete' — only ask about fields YOUR task depends on to do a "
    "good job. For example, an architecture reviewer given a canonical "
    "intake summary that happens to include 'Cost Center: UNKNOWN' "
    "should not ask about the cost center — that has nothing to do with "
    "evaluating architecture notes against standards, so it's just "
    "unused background noise, not a gap in your inputs.\n\n"
